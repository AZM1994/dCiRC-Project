...
global Lee_CIRC_data omega_DD T_interval Phase_Data days_P number_of_timesteps cost_range T_interval_stable...
       alpha_1 alpha_2 alpha_3 alpha_4 target_C_ZG target_ke target_s target_a
%% Load normalized race tube data
Lee_CIRC_data   = load('frq7_VI_Normalized.mat');
Lee_CIRC_data   = Lee_CIRC_data.data_saved;
cost_range      = 1:656;
days_P          = Lee_CIRC_data(:,1);% - Lee_CIRC_data(1,1);
Phase_Data      = Lee_CIRC_data(:,2);
...
for ii = 1 : number_of_timesteps - 1
	T_interval(ii) = Lee_CIRC_data(ii+1,1) - Lee_CIRC_data(ii,1);
end                            
...
alpha_3             = 0.01; % paramter for convex cost function
alpha_4             = 0.01; % paramter for convex cost function
target_C_ZG         = 0.0432; % target zeitgeber strength
target_ke           = 0.0972; % target elastic factor
target_s            = 0; % target shape factor of CiRC
target_a            = 1; % target asymmetry factor of CiRC
tau_DD              = 30.0; % free running period for the selected genotype
...
% initial_swarm       = [0.0398994248890937,0,0.605100490920085,2,0];
options = optimoptions(@particleswarm,'PlotFcn','pswplotbestf','SwarmSize',100,'HybridFcn',@fmincon,...
          'FunctionTolerance', 10^-6, 'MaxStallIterations', 20, 'InitialSwarmMatrix',initial_swarm); % , 'Display','iter'
...
convex_cost = sqrt(sum((fitting_sin_curve(cost_range) - Phase_Data(cost_range)).^2)) ...
               + alpha_1 * (c_z - target_C_ZG)^2 ...
               + alpha_2 * (elastic_factor - target_ke)^2 ...
               + alpha_3 * (s - target_s)^2 ...
               + alpha_4 * (a - target_a)^2;
...
fitting_sincurve_P = theta_I_signal; 
    ...
    assignin('base','fitting_sincurve_P_cost_range',fitting_sincurve_P(cost_range))
    assignin('base','days_P_cost_range',days_P(cost_range))
    ...
    figure(1);
        ...
        xticks(25.54:12:121.54)
        xticklabels({'ZT0','ZT12','ZT0','ZT12','ZT0','ZT12','ZT0','ZT12','ZT0'})
        yticks(0.2:0.1:0.3)
        yticklabels({' ',' ',' '})
        set(gca,'fontsize',36);
        ax1 = gca; % current axes
        ax1.XAxisLocation = 'top';
        xlim([days_P(17) days_P(end)])
    ...
    figure(2);
        ...
        area(days_P,zeros(number_of_timesteps,1) + 1, 'FaceColor', 'k', 'FaceAlpha', 1, 'LineStyle','none')
        area(days_P, light/c_z,'FaceColor', 'y', 'FaceAlpha', 1, 'LineStyle','none')
        alpha(0.3)
        xlabel('time/hours','Interpreter','latex')
        ...
        xticklabels({'24','36','48','60','72','84','96','108','120'})
        xlim([days_P(1) days_P(end)])
        ylim([min(nonzeros(omega_I)) - 0.02 max(omega_I) + 0.01])
        text(days_P(end)-5, 2*pi/zeit_t-0.01, '$\omega_T=\frac{2\pi}{T}$','Interpreter','latex','FontSize',42)
        text(days_P(end)-5, omega_DD+0.01, '$\omega_{I0}=\frac{2\pi}{\tau_{I0}}$','Interpreter','latex','FontSize',42)
        set(gca,'fontsize',36);
...
%% plot the best fitting dCIRC curve
    ...
    area(T_initial:T_interval:T_final_plot,zeros(number_of_timesteps,1) - 1, 'FaceColor', 'k', 'FaceAlpha', 1, 'LineStyle','none')
    area(T_initial:T_interval:T_final_plot, light/c_z,'FaceColor', 'y', 'FaceAlpha', 1, 'LineStyle','none')
    area(T_initial:T_interval:T_final_plot, -light/c_z,'FaceColor', 'y', 'FaceAlpha', 1, 'LineStyle','none')
    alpha(0.3)
    xlim([0 24])
    ylim([-1.005 1.005])
